...
export const setCookie: (key: string, value: string, duration?: number) => void =
  createIsomorphicFn()
    .client((key: string, value: string, duration = DEFAULT_DURATION) =>
      cookies.set(key, value, cookieOptions(duration)),
    )
    .server((key: string, value: string, duration = DEFAULT_DURATION) =>
      tsr_setCookie(key, value, cookieOptions(duration)),
    );
